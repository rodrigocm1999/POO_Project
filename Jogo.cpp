...
void Jogo::printGame(ostream &out) const {
    vector<Territorio *> kingdomTerritories = kingdom.getTerritories();
    out << "Turno : " << getTurn() << " \tFase : " << getPhase() << "\n";

    out << "Todos os territorios do Imperio (quantidade : " << kingdomTerritories.size() << " ) : \n";
    for (Territorio *territory : kingdom.getTerritories()) {
        out << "\t" << *territory << endl;
    }
    vector<Territorio *> worldTerritories = world.getTerritories();
    out << "------------------------------ \nRestantes territorios no mundo (quantidade : "
        << worldTerritories.size() << " ) : \n";
    for (Territorio *territory : world.getTerritories()) {
        out << "\t" << *territory << endl;
    }
    out << "------------------------------ \n";
